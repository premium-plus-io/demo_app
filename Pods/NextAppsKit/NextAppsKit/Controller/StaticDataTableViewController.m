...
- (void)reloadDataAnimated:(BOOL)animated {
    ...
    if (!animated) {
		...
		for (NSIndexPath *path in self.originalTable.deleteIndexPaths) {
			UITableViewCell *cell = [self.originalTable vissibleOriginalRowWithIndexPath:path].cell;
			
//			UITableViewCell *cell = [self.tableView cellForRowAtIndexPath:path];
			cell.layer.zPosition = -1;
			
			[self.tableView headerViewForSection:path.section].layer.zPosition = -1;
			[self.tableView footerViewForSection:path.section].layer.zPosition = -1;
		}
...
//		[self insertRowsAtIndexPathsCustom:self.originalTable.insertIndexPaths withRowAnimation:self.insertTableViewRowAnimation];
...
- (void)insertRowsAtIndexPathsCustom:(NSArray *)indexPaths withRowAnimation:(UITableViewRowAnimation)animation

{
	if (animation == UITableViewRowAnimationFade) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return [self.originalTable.sections[section] numberOfVisibleRows];
}
